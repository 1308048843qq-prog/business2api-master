...
// 获取临时邮箱（通过 Cloudflare 路由到 QQ 邮箱）
async function getTemporaryEmail(threadId) {
    const domain = process.env.CF_EMAIL_DOMAIN;
    if (!domain) {
        throw new Error('未配置 CF_EMAIL_DOMAIN 环境变量');
    }
    const localPart = `gm${Date.now()}${Math.floor(Math.random() * 10000)}`;
    const email = `${localPart}@${domain}`;
    console.log(`[线程 ${threadId}] 使用 CF 转发邮箱:`, email);
    return email;
}
...
async function getEmailContent(email, threadId, maxRetries = 20) {
    ...
    const imapHost = process.env.IMAP_HOST || 'imap.qq.com';
    const imapPort = parseInt(process.env.IMAP_PORT || '993', 10);

    if (!imapUser || !imapPass) {
        throw new Error('未配置 IMAP_USER / IMAP_PASS 环境变量');
    }
    ...
    for (let i = 0; i < maxRetries; i++) {
        let connection;
        try {
            console.log(`[线程 ${threadId}] IMAP 第 ${i + 1}/${maxRetries} 次检查验证码邮件...`);
            connection = await imaps.connect(config);
            await connection.openBox('INBOX');

            // 这里默认使用 UNSEEN 邮件，假设该 QQ 邮箱专用于接收验证码
            ...
            if (messages.length === 0) {
                console.log(`[线程 ${threadId}] 暂未收到新邮件，5 秒后重试...`);
            } else {
                ...
                const mail = await simpleParser(idHeader + (all && all.body ? all.body : ''));

                const subject = mail.subject || (headerPart && headerPart.body.subject ? headerPart.body.subject[0] : '');
                const content = `${mail.text || ''}\n${mail.html || ''}`;

                return { subject, content };
            }
        } catch (error) {
            console.log(`[线程 ${threadId}] IMAP 尝试 ${i + 1}/${maxRetries} 失败:`, error.message);
            try {
                if (connection) {
                    await connection.end();
                }
    ...
    throw new Error('无法通过 IMAP 获取邮件内容');
}
